...
- 🖼️ **插件下载后转 WebP 再发**：从帖子 `content` 的 `<img src>` 解析图片，`/mw600/`、`/mw1024/` 升为 `/large/`，带 Referer 并发下载，静态图用 WASM（无 sharp / 无 ImageScript 原生 .node）转成 WebP 后再交给适配器
- 🚀 **并发预上传后再合并转发**：OneBot（LLOB）先把每张图并发私聊发给自己（每条 `send_private_msg` 协议端可并行处理），再用这些消息 ID 打成一条「标题 + 一图一节点」的合并转发，最后撤回私聊里的临时图
- 🪵 **调试日志**：默认打开，控制台会打拉取、转码体积、每张上传耗时和 message_id。测完可在配置里关掉 `debug`
- ⏳ **先发「正在解析」**：下载、转码完成后再发合并转发，提示默认不撤回。发送时把 OneBot `responseTimeout` 临时调到 10 分钟，避免协议端还在传图时适配器 60 秒误报超时
...
当前版本 **1.2.3**：仍是一条合并转发。静态图用 WASM 转 WebP。OneBot（LLOB）并发把图片私聊发给自己，再按消息 ID 组成「标题 + 一图一节点」的正式转发。调试默认打开。下载前先发「正在解析煎蛋热榜…」，发出后不撤回。
