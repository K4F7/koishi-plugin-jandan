...
- ⏳ **先发「正在解析」**：下载、转码完成后再发合并转发，发出后撤回提示。OneBot 合并转发超时不再跟一条失败提示（协议端可能仍会发出）
...
当前版本 **1.2.1**：仍是一条合并转发。静态图用 WASM 转 WebP 再发（不再走 ImageScript 原生 `.node`，Alpine 上不会静默回退 JPEG）。默认跳过 GIF；单张上限 8MB。下载前先发「正在解析煎蛋热榜…」，发完撤回。
