...
- 🚀 **同节点并发上传后再换成正式转发**：OneBot 先发一条「同一节点多图」的合并转发（协议端会并发传图），读出图床地址后发「标题 + 一图一节点」的正式转发，成功再撤回临时那条
...
当前版本 **1.2.4**：仍是一条合并转发。静态图用 WASM 转 WebP。OneBot 先发同节点多图（单个气泡并发上传），再换成标题 + 一图一节点的正式转发并撤回临时消息。下载前先发「正在解析煎蛋热榜…」，发出后不撤回。
